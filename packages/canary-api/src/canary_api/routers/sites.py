...
from canary_api.schemas.site import (
    SiteCreate,
    SiteCreateResponse,
    SiteResponse,
    SiteUpdate,
...
def _generate_site_key(environment: str) -> str:
    """Generate a unique site key like cy_live_XXXX or cy_test_XXXX."""
    suffix = secrets.token_hex(12)
    prefix = "cy_live" if environment == "live" else "cy_test"
    return f"{prefix}_{suffix}"


def _generate_api_key() -> str:
    """Generate a raw API key like cy_sk_XXXXXXXX."""
    return f"cy_sk_{secrets.token_hex(24)}"
...
async def create_site(
    ...
    api_key_prefix = raw_api_key[:11]  # cy_sk_XXXXX
