...
export function createDOMPayload(method: DOMDeliveryMethod, content: string, testId: string): Element {
  switch (method) {
    ...
    case 'form_hidden_field': {
      ...
      form.setAttribute('data-canary-test', testId);
      form.setAttribute('aria-hidden', 'true');

      const input = document.createElement('input');
      input.type = 'hidden';
      input.name = `canary_${testId}`;
      input.value = content;
    ...
    case 'data_attribute': {
      ...
      el.style.display = 'none';
      el.setAttribute('data-canary-test', testId);
      el.setAttribute('data-canary-instruction', content);
      return el;
    }
    ...
    case 'svg_text': {
      ...
      svg.setAttribute('data-canary-test', testId);
