...
export function createStylePayload(method: StyleDeliveryMethod, content: string, testId: string): Element {
  ...
  switch (method) {
    case 'css_pseudo_element': {
      ...
      const className = `canary-pseudo-${safeTestId}`;

      // Create the style element with the ::after rule
      const style = document.createElement('style');
      style.setAttribute('data-canary-test', `${safeTestId}-style`);
      ...
      el.setAttribute('data-canary-test', safeTestId);
    ...
    case 'image_alt_text': {
      ...
      img.setAttribute('data-canary-test', safeTestId);
