...
export function createMetaPayload(method: MetaDeliveryMethod, content: string, testId: string): Element {
  switch (method) {
    case 'meta_tag': {
      ...
      meta.setAttribute('name', `canarai-test-${testId}`);
    ...
    case 'json_ld': {
      ...
      script.setAttribute('data-canarai-test', testId);

      // Build the JSON-LD object safely — no raw string concatenation
      const jsonLd = {
        '@context': 'https://schema.org',
        '@type': 'WebPageElement',
        'name': `canarai-test-${testId}`,
        'description': content,
      };
    ...
    case 'microdata': {
      ...
      nameProp.setAttribute('content', `canarai-test-${testId}`);
